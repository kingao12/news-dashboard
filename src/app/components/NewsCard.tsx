...
const NewsCard = memo(({ item, onClick, isListMode = false }: {
  ...
  return (
    <motion.div
      layout
      initial={{ opacity: 0, y: isListMode ? 0 : 10, x: isListMode ? -4 : 0 }}
      animate={{ opacity: 1, y: 0, x: 0 }}
      whileHover={!isListMode ? { y: -4, borderColor: 'rgba(129, 140, 248, 0.4)' } : undefined}
      className={`${isListMode ? styles.cardList : styles.card} ${isUrgent ? styles.urgentBorder : ''} glass-card`}
